...
def cadastro(request):
    if request.method == 'GET':
        ...
        try:
            try:
                _ = User.objects.get(username=usuario)
                messages.add_message(request, constants.ERROR, 'Nome de usuario em uso!!')
                return redirect('/auth/cadastro')
            except:
                pass
            user = User.objects.create_user(
                ...
                is_active=False)
            user.save()
            token = sha256(f"{usuario}{email}".encode('utf-8')).hexdigest()
